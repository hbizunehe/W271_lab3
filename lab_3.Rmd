...
plots.ts.by.group <- function(var_name, group_name) {
plt <- main_df %>% mutate(factored=factor(main_df[[group_name]])) %>% 
    ggplot(aes_string(x='year', y=var_name)) + 
    geom_line(aes(color=factored))
  return((plt))
}
...
plots.box.by.group <- function(var_name, group_name) {
plt <- main_df %>% mutate(factored=factor(main_df[[group_name]])) %>% 
...
# Total fatalities
plt.tf.state = plots.ts.by.group('total_fatalities_rate', 'state_name')
plt.box = plots.box.by.group('total_fatalities_rate', 'speed_limit_70_plus')

# 
(plt.tf.state / plt.box)
```
