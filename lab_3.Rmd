...
plots.ts.by.group <- function(df, var_name, group_name) {
plt <- df %>% 
...
plots.box.by.group <- function(df, var_name, group_name) {
...
# Each line gets a color based on the group_name
# X-axis is always year
plots.scatter.by.state <- function(df, var_name) {
plt <- df %>% 
...
```{r, fig.width=7}
plt.ts = plots.ts.by.group(main_df, 'total_fatalities_rate', 'speed_limit')
plt.box = plots.box.by.group(main_df, 'total_fatalities_rate', 'speed_limit')
plt.ts | plt.box
```
> There is a general decrease in fatalities over time. There also an increase in highway speeds over time which seems to increase the road fatalities. 

### Seat belt requirements 

```{r, fig.width=7}
plt.ts = plots.ts.by.group(main_df, 'total_fatalities_rate', 'seat_belt_law')
plt.box = plots.box.by.group(main_df, 'total_fatalities_rate', 'seat_belt_law')

(plt.ts | plt.box)
```

> Seat belts started to become mandatory starting mis 80s and today there is only one state which does not have it as mandatory. Primary laws are the strictest and allow police to ticket drivers and passengers who are not wearing a proper safety restraint, even if that is the only traffic violation they are committing. Secondary seat belt laws, on the other hand, do not grant law enforcement officials the right to ticket drivers or passengers for failing to wear a safety restraint unless another traffic violation has occurred. There are 15 states with secondary seat belt laws. Source: https://www.cooper-law-firm.com/what-is-the-difference-between-primary-and-secondary-seat-belt-laws/

### Minimum drinking age

```{r, fig.width=7}
tmp_df <- main_df %>% 
  mutate(minimum_drinking_age=round(minimum_drinking_age, 0))

plt.ts = plots.ts.by.group(tmp_df, 'total_fatalities_rate', 
                           'minimum_drinking_age')
plt.box = plots.box.by.group(tmp_df, 'total_fatalities_rate', 
                             'minimum_drinking_age')
plt.scatter = plots.scatter.by.state(main_df, 'minimum_drinking_age')

(plt.ts | plt.box)/plt.scatter
```
> There is a general decline in fatality rate with increased minimum driking age. There is also an increase in minimum drinking age over time. 

### population_aged_14_to_24_rate

```{r, fig.width=7}
tmp_df <- main_df %>% mutate(population_aged_14_to_24_rate=
                               round(population_aged_14_to_24_rate, 0))
plt.ts = plots.ts.by.group(tmp_df, 'total_fatalities_rate', 
                           'population_aged_14_to_24_rate')
plt.box = plots.box.by.group(tmp_df, 'total_fatalities_rate', 
                             'population_aged_14_to_24_rate')
plt.scatter = plots.scatter.by.state(main_df, 'population_aged_14_to_24_rate')

(plt.ts | plt.box)/plt.scatter
```
> There is a general increase in fatality rate with a higher percentage of younger population. There is also a trend down of the younger population over time. 

### Graduated drivers licence law

```{r, fig.width=7}
tmp_df <- main_df %>% 
  mutate(graduated_drivers_license_law=round(graduated_drivers_license_law, 0))
plt.ts = plots.ts.by.group(
  tmp_df, 'total_fatalities_rate', 'graduated_drivers_license_law')

tmp_df <- tmp_df %>% filter(year>1992)
plt.box = plots.box.by.group(
  tmp_df, 'total_fatalities_rate', 'graduated_drivers_license_law')
plt.ts | plt.box
```
> The graduated drivers licence law was started to be introduced in the late 90's. The box plot, which has been filtered for years greater than 1992, suggests that even for that time frame, there is a reduction in fatality rate between the two groups.
...
plt.ts = plots.ts.by.group(
  main_df, 'total_fatalities_rate', 'per_se_law')
...
tmp_df <- main_df %>% 
  mutate(bac=ifelse(blood_alcohol_limit_10==1, '10', 
             ifelse(blood_alcohol_limit_08==1, '8', 'none'))) %>%
  mutate(bac=factor(bac, levels=c('8', '10', 'none')))

plt.ts = plots.ts.by.group(
  tmp_df, 'total_fatalities_rate', 'bac')
plt.box = plots.box.by.group(
  tmp_df, 'total_fatalities_rate', 'bac')
plt.ts | plt.box
```
...
### Miles drive per capita

```{r, fig.width=7}
plt.scatter.1 = plots.scatter.by.state(main_df, 'miles_driven_per_capita')
plt.scatter.2 = plots.scatter.by.state(main_df, 'unemployment_rate')
plt.scatter.3 = plots.scatter.by.state(main_df, 'minimum_drinking_age')

plt.scatter.1 / plt.scatter.2 / plt.scatter.3
```
