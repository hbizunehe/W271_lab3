...
df <- data %>%
  ...
  mutate(log_total_fatalities_rate = log(totfatrte)) %>%
  ...
  mutate(speed_limit_70_plus = ifelse(speed_limit_70_plus > 0.5, 1, 0)) %>%
  mutate(seat_belt_law = ifelse(seat_belt == 0, 'none',
...
The data contains state-level information on the total traffic fatalities rate in the specified year. The data also contains state law information that could have an impact on the traffic fatalities rate, like the speed limit, BAC legal limit, seat belt laws, etc. The data is likely a census that represents the entire population since it's on a yearly aggregated level. The outcome of interest, `total_fatalities_rate` is defined as the number of fatalities per 100,000 people.
...
main_plt
```
On the aggregated level, the fatality rate is going down accross the united states. Starting at around 25 traffic fatalities per 100,000 people in 1980 and reducing to around 17 traffic fatalities per 100,000 people in 2004.
...
Taking the log-transform of our outcome variable, `total_fatalities_rate` makes the distribution appear more normal.

```{r,  fig.align='center', fig.width=10, fig.height=12, echo=FALSE}
...
* We note that per-se law is a statistically significant parameter. The model suggests that having a per-se law is associated with a `r abs(round(coef(expanded.ols)['per_se_law1'], 2))` unit decrease in fatality rate as compared to not having per-se law.
...
stargazer(expanded.ols, expanded.within,
          ...
          style="qje", omit.stat=c("adj.rsq","f"), 
...
We note that per-se law is a statistically significant parameter. This model suggests that having a per-se law is associated with a `r abs(round(coef(expanded.within)['per_se_law1'], 2))` unit decrease in fatality rate as compared to not having per-se law. Compared to the OLS model which showed that having a per-se law is associated with a `r abs(round(coef(expanded.ols)['per_se_law1'], 2))` unit decrease in fatality rate as compared to not having per-se law

### What do you estimate for coefficients on primary seat-belt laws? How do the coefficients on primary seatbelt laws change, if at all?

In this model we note that seat belt primary law is a significant factor. This model suggests that having a primary seat belt law is associated with a `r abs(round(coef(expanded.within)['seat_belt_lawprimary'], 2))` unit decrease in fatality rate as compared to not having  primary seat belt law. Compared to the OLS model for which the parameter was not statistically significant and had a near-zero value of `r abs(round(coef(expanded.ols)['seat_belt_lawprimary'], 2))`
...
With a p-value of `r as.numeric(res$p.value)`, which is less than 0.05, we reject the null hypothesis of no serial correlation. This suggests that we must use robust standard errors for model parameters
