...
author: 'Aswin Thiruvengadam, Nathan Martinez, Haile Bizunehe, Hannah George, and Meera Sharma'
output:
    pdf_document: 
      toc: true
      toc_depth: 4
...
* We note that the seat belt law factors are not statistically significant. We also note that the coefficient of  `r abs(round(coef(expanded.ols)['seat_belt_lawprimary'], 4))` is practically small. 
...
# Pop Units in thousands; driving mile units in millions
us.pop.month <- 
...
fatal_rate_covid_max <- round(coef(expanded.within)['miles_driven_per_capita'] * max_miles_driven_pc, 3) 
fatal_rate_covid_min <- round(coef(expanded.within)['miles_driven_per_capita'] * min_miles_driven_pc, 3)

```

> 'The FE model suggests that a one mile increase in the miles driven per capita is associated with a `r format(round(coef(expanded.within)['miles_driven_per_capita'], 5), scientific = FALSE)` unit increase in the fatality rate. During the COVID boom, the largest increase in driving was on `r format(as.Date(largest.increase), "%B %Y")` and it results in a estimated increase of `r round(max_miles_driven_pc,2)` miles driven per capita and this is expected to result in a `r fatal_rate_covid_max` unit increase in the fatality rate.`

> 'The FE model suggests that a one mile increase in the miles driven per capita is associated with a `r format(round(coef(expanded.within)['miles_driven_per_capita'], 5), scientific = FALSE)` unit increase in the fatality rate. During the COVID boom, the largest decrease in driving was on `r format(as.Date(largest.decrease), "%B %Y")` and it results in a estimated increase of `r round(min_miles_driven_pc,2)`  miles driven per capita and this is expected to result in a `r fatal_rate_covid_min` unit decrease in the fatality rate.`
...
> 'With a p-value less than 0.05, we reject the null hypothesis of no serial correlation. This suggests that we must use robust standard errors for model parameters.'
...
pbg
```
> 'With a p-value of less than 0.05, we reject the null hypothesis and conclude that autocorrelation exists among the residuals at some order less than or equal to p.'


Using vcovHC, we calculate robust standard errors (white1), cluster robust standard errors (white2), arrelano standard errors (arrellano), and newey west standard errors (using vcovNW) for the coefficients within our FE model.
...
#expanded.model.se <- sqrt(diag(vcovHC(expanded.re, method = "white2", type = "HC0")))
