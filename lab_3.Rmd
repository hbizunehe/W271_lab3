...
<!-- As we have used an OLS regression for a time series, we expect the residuals to be correlated. We perform a Ljung-Box test to check this. The Null hypothesis is that the series is not autocorrelated and the alternate hypothesis is that the series is autocorrelated. We test with an of $\alpha=0.05$. -->
<!-- ```{r} -->
<!-- expanded.ols %>% residuals() %>% Box.test(type='Ljung-Box') -> lb.res -->
<!-- ``` -->
<!-- With a p value of `r lb.res$p.value` less than $\alpha$, we reject the Null hypothesis that the series is not auto-correlated. As the residuals are autocorrelated, the standard error of the parameters cannot be trusted. We review the model parameters with this in mind. -->
